 ...
 *     Currently no checking is done to determine whether the input is a valid coverage. This is
 *     because coverage validation involves segment intersection detection, which is much more
 *     expensive than the union phase. If the input is not a valid coverage then in some cases this
 *     will be detected during processing and a {@link TopologyException} is thrown. Otherwise, the
 *     computation will produce output, but it will be invalid. <p> Unioning a valid coverage
 *     implies that no new vertices are created. This means that a precision model does not need to
 *     be specified. The precision of the vertices in the output geometry is not changed.
 *
 * @author
 *   Martin Davis
 *
...
object CoverageUnion {
   ...
   */
  def union(coverage: Geometry): Geometry = {
    val noder = new SegmentExtractingNoder()
    // a precision model is not needed since no noding is done
    OverlayNG.union(coverage, null, noder)
  }
