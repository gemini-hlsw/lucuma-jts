...
public class BufferValidator
{
  ...
  private Map<String, Test> nameToTestMap = new HashMap<>();
  ...
  public void test() throws Exception {
    try {
      Collection<Test> tests = nameToTestMap.values();
      for (Iterator<Test> i = tests.iterator();
        i.hasNext();
        ) {
        Test test = i.next();
        test.test();
      ...
      public void test() throws Exception {
        if (getOriginal().getClass() == GeometryCollection.class) {
          ...
          Assert.assertTrue(
            ...
            BufferResultValidator.isValid0(getOriginal(), bufferDistance, getBuffer()));
      }
    });
  }
